...
import { createActionFromAuditAction, unlinkAuditActionAction } from "@/lib/actions/audits-revues";
...
export function AuditActions({ auditId, linked }: { auditId: string; linked: LinkedAction[] }) {
  ...
  async function handleCreate(event: React.FormEvent<HTMLFormElement>) {
    ...
    const result = await createActionFromAuditAction({
      auditId,
    ...
    setPending(false);
    if (result.ok) {
      toast.success("Action corrective créée et liée.");
      setCreating(false);
      router.refresh();
    } else {
  ...
  async function unlink(actionId: string) {
    setPending(true);
    const result = await unlinkAuditActionAction(auditId, actionId);
    setPending(false);
    if (result.ok) {
  ...
  return (
    <div className="flex flex-col gap-4">
      {linked.length === 0 ? (
        <p className="text-muted-foreground text-sm">Aucune action corrective liée à cet audit.</p>
      ) : (
      ...
      {creating ? (
        <form onSubmit={handleCreate} className="flex flex-col gap-3 rounded-lg border p-3">
          <Input
            ...
            required
            placeholder="Action corrective (écart à traiter)"
          />
