...
export function ReunionActions({
  reunionId,
  linked,
}: {
  reunionId: string;
  ...
  async function handleCreate(event: React.FormEvent<HTMLFormElement>) {
    ...
    const result = await createActionFromReunionAction({
      reunionId,
    ...
    setPending(false);
    if (result.ok) {
      toast.success("Action créée et liée à la réunion.");
      setCreating(false);
      router.refresh();
    } else {
  ...
  async function unlink(actionId: string) {
    setPending(true);
    const result = await unlinkReunionActionAction(reunionId, actionId);
    setPending(false);
  ...
  return (
    <div className="flex flex-col gap-4">
      {linked.length === 0 ? (
        <p className="text-muted-foreground text-sm">Aucune action issue de cette réunion.</p>
      ) : (
