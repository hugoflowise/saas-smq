...
import { deleteDomaineVersionAction } from "@/lib/actions/domaine";
import { formatDate } from "@/lib/format";
import { useReadOnly } from "@/lib/hooks/read-only-context";
import { type DomaineSnapshot, DomaineSnapshotView } from "./domaine-snapshot";

export type DomaineVersionItem = {
  id: string;
  version: string;
  publishedAt: string | null;
  publisher: string | null;
  snapshot: DomaineSnapshot | null;
};

/** Historique des versions figées du domaine d'application. */
export function DomaineVersionHistory({ versions }: { versions: DomaineVersionItem[] }) {
          ...
          <div className="flex shrink-0 items-center gap-1">
            <Dialog open={openId === v.id} onOpenChange={(o) => setOpenId(o ? v.id : null)}>
              ...
              <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-3xl">
                <DialogHeader>
                  <DialogTitle>
                    Domaine d'application · version {v.version} ({formatDate(v.publishedAt)})
                  </DialogTitle>
                </DialogHeader>
                {v.snapshot ? (
                  <DomaineSnapshotView snapshot={v.snapshot} />
                ) : (
...
function SupprimerVersionButton({ version }: { version: DomaineVersionItem }) {
  ...
  async function supprimer() {
    setPending(true);
    const r = await deleteDomaineVersionAction(version.id);
    setPending(false);
    if (r.ok) {
