...
import { deletePartiesPrenantesVersionAction } from "@/lib/actions/parties-prenantes";
import { formatDate } from "@/lib/format";
import { useReadOnly } from "@/lib/hooks/read-only-context";
import {
  type PartiesPrenantesSnapshot,
  PartiesPrenantesSnapshotView,
} from "./parties-prenantes-snapshot";

export type PartiesPrenantesVersionItem = {
  id: string;
  version: string;
  publishedAt: string | null;
  publisher: string | null;
  snapshot: PartiesPrenantesSnapshot | null;
};

/** Historique des versions figées de la cartographie des parties prenantes. */
export function PartiesPrenantesVersionHistory({
  versions,
}: {
  versions: PartiesPrenantesVersionItem[];
}) {
          ...
          <div className="flex shrink-0 items-center gap-1">
            <Dialog open={openId === v.id} onOpenChange={(o) => setOpenId(o ? v.id : null)}>
              ...
              <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-4xl">
                <DialogHeader>
                  <DialogTitle>
                    Parties prenantes · version {v.version} ({formatDate(v.publishedAt)})
                  </DialogTitle>
                </DialogHeader>
                {v.snapshot ? (
                  <PartiesPrenantesSnapshotView snapshot={v.snapshot} />
                ) : (
...
function SupprimerVersionButton({ version }: { version: PartiesPrenantesVersionItem }) {
  ...
  async function supprimer() {
    setPending(true);
    const r = await deletePartiesPrenantesVersionAction(version.id);
    setPending(false);
    if (r.ok) {
