...
import { appliquerLibelles, clauseBadge, referentielsBadge } from "@/lib/normes-libelles";
...
export async function PageHeader({
  ...
  // Normes chargées une seule fois si nécessaire : pour résoudre le badge depuis
  // le concept, et/ou substituer les jetons {{…}} des textes.
  const helpString = typeof help === "string" ? help : "";
  const needsNormes =
    (!!concept && !isoClause) ||
    [title, description ?? "", helpString].some((s) => s.includes("{{"));
  const normes = needsNormes ? await getNormesActives() : [];
  ...
  const titleR = title.includes("{{") ? appliquerLibelles(title, normes) : title;
  const descriptionR = description?.includes("{{")
    ? appliquerLibelles(description, normes)
    : description;
  const helpR =
    typeof help === "string" && help.includes("{{") ? appliquerLibelles(help, normes) : help;
  return (
    <div className="mb-6 flex flex-col gap-3">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <h1 className="font-semibold text-2xl tracking-tight">{titleR}</h1>
            {badge ? (
          ...
          {descriptionR ? (
            <p className="mt-1 text-muted-foreground text-sm">{descriptionR}</p>
          ) : null}
        </div>
        {children ? (
      ...
      {helpR ? (
        <details className="group rounded-lg border bg-surface text-sm">
          ...
          <div className="border-t px-3 py-2.5 text-muted-foreground leading-relaxed">{helpR}</div>
        </details>
      ) : null}
    </div>
  );
